...
- Landing page (`landing-page`, :9000): a public front door for the demo that explains what GBO is, sketches the three flows (DvTP, EUDI, OOTS) over the shared pipeline, and links into the other front-ends, the FSC-controllers and the observability stack. Static React/Vite app with no backend; every outbound link is resolved at runtime (`VITE_*` on the dev-server, `runtime-config.js` via envsubst in the nginx image) so the same image serves both the local stack and a deployed environment.
