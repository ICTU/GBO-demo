...
escaped_graphql_public_url="$(escape_json_string "${GRAPHQL_PUBLIC_URL:-${VITE_GRAPHQL_PUBLIC_URL:-}}")"

printf 'window.__GBO_RUNTIME_CONFIG__ = {"eudiPublicUrl":"%s","eudiClientId":"%s","jaegerPublicUrl":"%s","grafanaPublicUrl":"%s","graphqlPublicUrl":"%s"};\n' \
  "$escaped_eudi_public_url" \
  "$escaped_eudi_client_id" \
  "$escaped_jaeger_public_url" \
  "$escaped_grafana_public_url" \
  "$escaped_graphql_public_url" \
