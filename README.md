...
Do **not** pass `--reader-origin "$EUDI_READER_ORIGIN_URL"` unless that variable genuinely equals `EUDI_PUBLIC_URL`. The `requestOriginBaseUrl` baked into a reader certificate is checked by the wallet against the origin it actually reached, so it has to be the URL the wallet talks to — which is `EUDI_PUBLIC_URL`, and the script's default. Getting this wrong fails in a way that reads like a certificate problem but is not: the certificate chain, EKU and `client_id` all validate, the wallet then aborts the session with `access_denied`, and the server records a **CANCELLED** session with no error text. A rejected certificate would instead give `invalid_request` and a **FAILED** session carrying the reason — so `CANCELLED` with a clean server log points at the origin, not the chain. The frontends need no separate change: they derive the same `client_id` from `EUDI_PUBLIC_URL`.
