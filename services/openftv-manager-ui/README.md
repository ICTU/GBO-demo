...
`FTV_MANAGER_DEPLOY_PASSWORD` before running `make demo-manager`
(`FTV_POSTGRES_PASSWORD` is needed by every demo target, not just this one). Store them as CI/CD secrets; none belongs in frontend
