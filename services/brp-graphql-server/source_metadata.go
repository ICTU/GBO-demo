...
func newSourceMetadataPublisher(payload, rawPrivateJWK []byte) (*sourceMetadataPublisher, error) {
	...
	protected, err := json.Marshal(map[string]any{
		"alg": "EdDSA", "kid": sourceMetadataBase64URL.EncodeToString(thumbprint[:]), "typ": "gbo-attestations+jws",
	...
	return &sourceMetadataPublisher{compactJWS: []byte(compact), etag: `"` + fmt.Sprintf("%x", digest) + `"`}, nil
}
