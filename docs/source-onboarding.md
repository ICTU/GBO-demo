...
- `onboard-source --env local` herhaalt dezelfde validatie, maakt lokale
  issuer-, reader- en statuscertificaten, publiceert immutable Type Metadata en
...
`.local/secrets/<oin>/issuance.env`. `make eudi-config` leest dit bestand na de
...
De lokale certificaatprovider is uitsluitend voor ontwikkeling. Een
productieprovider en CI-automatisering volgen pas nadat dezelfde flow lokaal
end-to-end is bewezen en het CA-/trustmodel met het Wallet-team is vastgesteld.
